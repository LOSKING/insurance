...
import com.bjgoodwill.insurance.lis.dao.LisDetailInfoDao;
...
/**
 * @Package: com.bjgoodwill.insurance.lis.service.impl
 * @Decription {42a7c0d6-4438-4662-98f4-d166c3d78514} 检验详细信息ServiceImpl
 * @author liuxch 
 * @date 2018年3月27日 下午5:15:38
 */
@Service 
//{806ebdad-8af3-46e7-bf18-5e5eb1dd3256} modifly LisInfoServiceImpl继承BaseServiceImpl
public class LisDetailInfoServiceImpl extends BaseServiceImpl <LisDetailInfoModel> implements LisDetailInfoService {

	@Autowired
	private LisDetailInfoDao dao = null;
	
	@Override
	public String getLisDetailInfoByPatientIdToView(JSONObject jsonObject) {
		String patientId = jsonObject.getString("patientId");
		if (StringUtils.isEmpty(patientId)) {
            throw new BusinessException(BizExceptionEnum.REQUEST_PARAMS_INCOMPLETE, "patientId参数不能为空");
        }
		List<LisDetailInfoModel> model =  dao.getLisDetailInfoByPatientIdToView(patientId);
	...
	@Override
	public String getLisDetailInfoByPatientIdToSetting(SettingModel setting,JSONObject jsonObject) {
		String patientId = jsonObject.getString("patientId");
		if (StringUtils.isEmpty(patientId)) {
            throw new BusinessException(BizExceptionEnum.REQUEST_PARAMS_INCOMPLETE, "patientId参数不能为空");
        }
	...
	@Override
	public String getLisDetailInfoByPatientIdToSettingList(List<SettingModel> settings,JSONObject jsonObject) {
		String patientId = jsonObject.getString("patientId");
		if (StringUtils.isEmpty(patientId)) {
            throw new BusinessException(BizExceptionEnum.REQUEST_PARAMS_INCOMPLETE, "patientId参数不能为空");
        }
	...
	public String getModelInfo(String interfceMode, JSONObject requestJson, List<SettingModel> settingModels) {
		if(SystemConstant.VIEW_MODEL.equals(interfceMode)){
			//视图方式查询数据
			return this.getLisDetailInfoByPatientIdToView(requestJson);
		}else if (SystemConstant.HTTP_MODEL.equals(interfceMode)){
			//通过WebService方式查询数据
			if(settingModels.size() == 1){
				//settingModel查询数据
				return this.getLisDetailInfoByPatientIdToSetting(settingModels.get(0), requestJson);
			}else{
				return this.getLisDetailInfoByPatientIdToSettingList(settingModels, requestJson);
			}
		}
		return "";
	}
